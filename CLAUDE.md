...
- Banner (`PWAInstallPrompt`) shows on Android even **without** `beforeinstallprompt` (Samsung/Firefox often never fire it). Add when the prompt exists; otherwise How → `/settings#android-install`. Hidden in standalone. Dismiss key `adhx-a2hs-dismissed`.
...
- Playback: `Space` play/pause (`theater-toggle-play`), `M` mute, `E` expand (hide/show chrome), `R` cycle repeat
