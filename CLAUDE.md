...
- **Routes**: `GET /api/auth/me` (account view: `{ authenticated, user, identities: { x: { username, avatarUrl }, email }, xConnected, isAdmin }` — the client-side source of truth; Header/AuthedHome/preferences use it, NOT the twitter/status route. `identities.x.avatarUrl` is the X profile photo (from oauth tokens, falling back to `users.avatarUrl`). Account chrome honors `user_preferences.avatarSource` (`'x'` | `'generated'`). `isAdmin` is true only when the username is in `ADMIN_USERNAMES`. Banned sessions return the signed-out shape.), `POST /api/auth/email/request` (rate-limited 60s/email, no user enumeration), `GET /api/auth/email/callback?token=` (signin sets session; change relinks email), `POST /api/auth/email/change` (authed), `POST /api/auth/twitter/disconnect` (409 when it's the last identity), `POST /api/auth/logout`.
- **X start + callback**: `GET /api/auth/twitter` requires a session; unsigned visitors redirect `/?auth_error=x_link_only`. The callback calls `findOrCreateUserForX(x, existingSession.userId)`: signed-in users get X **linked** for bookmark sync; no session or `sign_in_required` → `/?auth_error=x_link_only` (no user created); an X identity already on another account redirects `/settings?auth_error=x_already_linked` without touching the session. Tokens are saved under the app userId. After a successful link, honor `adhx_return_url` if safe, else land on `/settings` (not `/?firstLogin=true` — signed-in `/` redirects to `/live` and drops the query).
- **Status route semantics** (`/api/auth/twitter/status`): `authenticated` = valid session + users row (independent of X). A **fatal token-refresh error deletes the X tokens but keeps the session** — the account outlives the X connection; `xConnected`/`needsReconnect` describe the X state. Sync for users without X tokens surfaces SSE `code: 'reauth'` with a Connect-with-X prompt (already-signed-in reconnect, not sign-in).
...
- Theater: `1` Live, `2` Saved (signed-in personal theater and signed-in shared previews; no-op signed-out / playlists). `Q` Queue (the full playlist: desktop panel / mobile up-next sheet). While it's open, `↓`/`↑` move through the list (they do not scroll the stage), Enter plays the focused row, Esc or a click outside closes. Live and Saved queue types (Videos / Photos / Text / Articles) are a multi-select in Queue / the up-next sheet, persisted as `adhx-theater-types`. Playlists omit the pills. The dock end cap is Queue / leftover-run progress (`N in queue` until the first leave, then `16 of 23`) / `N new`. Catching up closes that leftover run — a later arrival is `1 in queue`, not `2 of 3` from the finished run. A mid-play Live prepend keeps `played` from `countPlayedThisRun` (still `N in queue` until something is left) — do not treat the bumped playlist index as played. Saved Play once is the 1-based now-playing index (`2 of 92` on the second post), not leftover-run math. Keep playing is `23 on repeat`. Repeat this post is `1 on repeat`. Position (`3/7`) is not shown on Live — Live always stages the head of the leftover stack. An active filter keeps the toggle labelled Queue and adds a clay ListFilter cue (types stay in the overlay; hover title still names them)
...
- Also: `.` menu (arrows / J K move, Enter opens the focused item), `Q` Queue, `W` Re-watch all, `P` Keep playing (caught-up stage), `?` (Shift+/) help overlay (`TheaterShortcutsHelp`), `Esc` close help / Queue / collection
...
| `/live`       | signed-in Live tab (community pulse) — leftover unseen. Signed-out: redirects to `/`         |
...
The Live ⇄ Saved switch is a pair of ROUTES (`/live` ⇄ `/saved`), not local state, so each side is linkable and survives a reload — `TheaterShell`'s `onPersonalTabChange` flips the tab locally (instant) then the page navigates. A Live caught-up stay (`waiting`) survives that flip so coming back does not keep playing the Saved clip on the shared `<video>`. `theater-resume` still fires for Saved, but StageVideo no-ops while `covered` (Live waiting / Saved All Clear / a non-video stage). Live auto-advance (`onEnded` / `theater-advance`) marks the leaving post seen — clips shorter than the 2s dwell used to stay unseen. Saved All Clear is a z-10 overlay; `<Stage/>` stays mounted (`covered`) so the iOS unmute grant survives. A Saved prepend after All Clear jumps to index 0 (not `length + 1`). Live Save prepends with `idPlatform`, same as paste. Read (`articleMode`) resets on the staged identity, not `personalIndex`. Live poll runs as soon as `live` becomes true. Repeat/queue-type prefs, membership lookup, and transport live in dedicated hooks; leftover counts use `wasSeenBeforeLeftoverRun`, Queue headings use `wasSeenOnEntry`. Live `replaceState`s the address bar onto the staged post (same as signed-out `/`); Saved stays on `/saved`, and flipping back writes `/saved` even if Live left a preview path in the bar. `TheaterShell` snapshots `personalItems` at mount, so `/saved` fetches the queue BEFORE mounting the shell; `/live` never waits on it. The playing Saved identity is written to `sessionStorage` (`adhx-saved-playing`) so a `/live` ⇄ `/saved` remount (or a Live paste that prepends) lands on the same post — but only when every earlier leftover in this run (`adhx-saved-played`) has already been left. Otherwise Saved starts at the first leftover (Videos does not walk forward past unchecked rows). `?open=` still wins (`preserveSavedStart` skips the hydrate leftover clamp so a library tap is not snapped to the first unread). TheaterShell leftover-clamps on remount without open, and on an in-session Queue type change. Signed-in `/` is Live (the hook after a preview); `/saved` is the unread pile you come back to.
...
- **Preview pages ARE the theater** (Phase 3): the five preview page routes keep crawler SEO in `generateMetadata` (that path still awaits the upstream). The **page RSC does not wait** on FxTwitter / a scrape / oEmbed — it paints a URL stub + TheaterShell immediately and passes a `sharedResolve` Promise (`src/lib/theater/resolve-shared-preview.ts`). JSON-LD + `SharedPostStatic` stream in a Suspense sibling; TheaterShell `replaceItem`s the stub in place. Tweets show `StageResolving` until the kind is known; Reels / TikTok / Shorts are already video so the stage plays while caption/author fill in. Reels / TikTok / Shorts share `SharedPreviewPage` + `getSavedPreviewDisplay` + `recordHumanPreview`. `buildSharedSeed()` (`src/lib/theater/shared-seed.ts`) pins the shared post as the lead item and copies pulse-only `addedAt` / `saveCount` / `trendCount` onto it so the flame chip shows on first paint. Shared-mode dock: "This post" heading in Queue (the post the visitor opened) until this session watches it — then it joins Watched earlier and leftover copy does not stay on `1 in queue` at caught-up. "More being sent right now" header, authed Save POSTs `/api/bookmarks/add` with a `sourceUrl()`-reconstructed canonical URL (NOT `item.url`, which for pulse items is the on-ADHX preview path). A signed-in new-open autosaves the lead and does **not** shared-post-repeat: the opened post is first in the leftover Live run (everything still unseen), then auto-advances. Signed-out visitors keep the pin until they Next. Signed-in previews keep shared Save/Tag + paste chrome, and also get the Live ⇄ Saved cluster (Live is current; Saved → `/saved`; Close → `/library` on desktop). Mobile omits Close — the avatar menu already sits top-right and has Library. Do **not** pass `personalChrome` here, or the shared Save pill becomes the live-tab pair. The `*PreviewLanding` components were unmounted then deleted (dead-code cleanup, 2026-08-21).
...
- Theater-dark theme default (unset `localStorage.theme`) now covers `/`, `/live`, `/saved`, `/trending`, and `/trending/*` — `resolveInitialTheme` + the layout FOUC script stay in lockstep.
