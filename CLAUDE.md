...
| Route         | What renders                                                                                                                            |
| ------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `/`           | signed-out: public live theater + crawlable static list. Signed-in: **redirects to `/collection`** (next unread is the default landing) |
| `/live`       | signed-in Live tab (community pulse). Signed-out: redirects to `/`                                                                      |
| `/collection` | My Collection — your unread queue as a playlist. Signed-in home                                                                         |
| `/library`    | the grid: `AuthedHome`, three view modes, FilterBar, search, tags                                                                       |

The Live ⇄ My Collection switch is a pair of ROUTES (`/live` ⇄ `/collection`), not local state, so each side is linkable and survives a reload — `TheaterShell`'s `onPersonalTabChange` flips the tab locally (instant) then the page navigates. `TheaterShell` snapshots `personalItems` at mount, so `/collection` fetches the queue BEFORE mounting the shell; `/live` never waits on it. Signed-in `/` cannot be Live — that would bounce the Live tab back to unread.
...
- Theater-dark theme default (unset `localStorage.theme`) now covers `/`, `/live`, `/collection`, `/trending`, and `/trending/*` — `resolveInitialTheme` + the layout FOUC script stay in lockstep.
