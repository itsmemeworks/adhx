...
- **Fix**: `translate="no"` on `<html>` + `<meta name="google" content="notranslate">` in `layout.tsx` — the app is interactive React on every route, so the offer is suppressed sitewide rather than per-surface. Verified in-browser: `document.documentElement.translate === false` and inherited `false` deep inside the theater. Additionally `TheaterLinkedText` now wraps each text run in a `<span>` (bare text nodes between `<a>`/`<br>` siblings were the most exposed spot) — regression-tested by the simulated translate pass, which throws on the pre-fix code and passes after. **NOT fixed**: a translation _extension_ ignores the attribute and can still crash other chrome (that `<Download>` button); full immunity would mean wrapping every text run app-wide — deliberately not attempted.
...
- **State**: 2270 tests green (181 files; new `sync-added-at.test.ts`, `TheaterLinkedText-translate.component.test.tsx`), typecheck/lint/format clean. Follow-up: the sync route's stamp wiring has no route-level test (no sync harness exists) — covered only by the pure helper's tests.
...
- **Modularity**: shared `fetchWithTimeout()` (25 call sites); `feed/utils.tsx` → device/media-actions/text-rendering + facade; AuthedHome → `useSyncListener`/`useTriageQueue`; TheaterShell (1483→1335) → `useTheaterKeyboard`/`useTheaterPrefetch`/`useTheaterDwell` + `stage-primitives.tsx` (StageIconButton/Headline/CTA/Frame); `saveBookmark`→`lib/sync/save-bookmark.ts`, `buildFeedItem`→`lib/feed/build-item.ts`; Sentry-visible `handleRouteError` in preferences/collections/sync-logs/auth-status routes.
