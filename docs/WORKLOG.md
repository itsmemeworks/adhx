...
- **State**: 2176 tests green (174 files), typecheck/lint/format clean; live-browser verified at mobile width (17-item wrap, waiting+replay, copy, author hrefs, 32px chips, hover-gate CSS confirmed in compiled bundle). Follow-ups: owner re-test on device (YT progress on autoplay; whether non-YT items also lose sound after the waiting stage — if so capture `?avdebug=1`); audit pre-existing `clay/NN` opacity-modifier classes; signed-out checks for the menu current-dot on `/` + leaderboard burger ran only as component tests (env browser was signed in).
