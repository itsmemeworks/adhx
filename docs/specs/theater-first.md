...
| Preview pages (`/{user}/status/{id}`, `/reels/{id}`, `/@{user}/video/{id}`, `/shorts/{id}`) | Same theater seeded at the shared post; chrome paints on a URL stub while FxTwitter / a scrape / oEmbed streams in. "This post" heading in Queue, post meta pinned to stage, dock shows "More being sent right now" queue; SEO markup streams in a Suspense sibling, `generateMetadata` still awaits for crawlers |
