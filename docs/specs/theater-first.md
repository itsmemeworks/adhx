...
- **Autoplay**: playback defaults muted. Signed-in users can enable **Sound on by default** in Settings; the account preference remains the fallback, while the latest audio-button choice is stored in `localStorage` and synchronized across tabs. Mobile browsers can still reject audible autoplay on a fresh document, so synchronized/new-link unmute attempts safely fall back to muted playback and a stage tap retries the saved choice inside the user gesture. The rail/dock audio button remains the explicit control.
