...
export async function GET(request: NextRequest) {
  ...
  if (!code || !state) {
    return NextResponse.redirect(
      new URL('/?error=Missing%20code%20or%20state', request.url)
    )
  }
  ...
  try {
    ...
    if (!codeVerifier) {
      return NextResponse.redirect(
        new URL('/?error=Invalid%20or%20expired%20state', request.url)
      )
    }
    ...
    if (returnUrl && returnUrl.startsWith('/')) {
      ...
      redirectUrl = new URL('/?firstLogin=true', request.url)
    }
