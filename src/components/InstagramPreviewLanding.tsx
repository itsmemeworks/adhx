...
/**
 * ReelShareButton — floating share/download control overlaid on the Reel video.
 *
 * Matches the interaction pattern of TweetPreviewLanding's MediaShareButton:
 * - Mobile: always visible, Share icon, opens native share sheet
 * - Desktop: hover-reveal, Download icon, triggers server-side download
 * - Shows Loader2 / Check transition states
 */
  ...
  const handleClick = async (e: React.MouseEvent) => {
    ...
    try {
      if (isMobile && typeof navigator.share === 'function') {
        ...
        // Desktop: trigger server download via hidden anchor
...
/**
 * ConnectCta — right-column primary CTA for unauth users.
 * Mirrors the unauthenticated TweetPreviewLanding CTA shape.
 */
function ConnectCta({
  connecting,
  onConnect,
